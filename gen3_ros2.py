...
class OpenVLAClient_ImageSub(Node):
    def __init__(self, instruction=None):
        ...
        # Publisher
        self.control_topic = "/joint_trajectory_controller/joint_trajectory"
        self.joint_puber = self.create_publisher(JointTrajectory, self.control_topic, 10)
        self.processing_timer = self.create_timer(2.0, self.action_run)
    ...
    def action_run(self,):
        ...
        trajectory_msg.joint_names = ['joint_1', 'joint_2', 'joint_3', 'joint_4', 'joint_5', 'joint_6', 'joint_7']  # Replace with your joint names

        # Create a trajectory point
        point = JointTrajectoryPoint()
        point.positions = [0.0] * len(self.action) 
        for i in range(len(self.action)):
            point.positions[i] = self.action[i]
        point.time_from_start.sec = 2  # Set duration for this point

        # Add the point to the trajectory message
        trajectory_msg.points.append(point)

        # Publish the message
        self.joint_puber.publish(trajectory_msg)
